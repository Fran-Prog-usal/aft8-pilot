...
def write_report(directory: Path):
    ...
    ax.bar(
        positions - 0.18,
        entropy.metric_hits / entropy.n_texts,
        0.36,
        label="Métrica",
        color="#286682",
    )
    ax.bar(
        positions + 0.18,
        entropy.baseline_hits / entropy.n_texts,
        0.36,
        label="Entropía",
        color="#cf8c39",
    )
    ax.set_xticks(
        positions, [f"{labels[row.metric]} (n={row.n_texts})" for row in entropy.itertuples()]
    )
    ax.set_ylim(0, 1)
    ...
    ax.errorbar(
        ...
        xerr=np.vstack([values - factorial.ci95_low, factorial.ci95_high - values]),
        fmt="o",
        color="#286682",
        capsize=4,
    )
    ...
    text = [
        "# Reanálisis estadístico del piloto de Mistral",
        "",
    ...
    for row in global_scores.itertuples():
        text.append(
            f"- {row.mode}: {row.hits}/{row.n_score_rows} aciertos ({row.hit_rate:.2%}), procedentes de {row.n_texts} textos. El resumen global es descriptivo; sus filas no son observaciones independientes."
        )
    text += [
        "",
        "| Métrica | Aciertos | Entropía | p bilateral | p BH |",
        "|---|---:|---:|---:|---:|",
    ]
    for row in entropy.itertuples():
        text.append(
            f"| {labels[row.metric]} | {row.metric_hits}/{row.n_texts} | {row.baseline_hits}/{row.n_texts} | {row.p_raw:.5f} | {row.p_bh:.5f} |"
        )
    text += [
        "",
        "McNemar exacto usa los pares discordantes. BH se aplica a las 16 comparaciones de cada modo. Los resultados se interpretan como exploratorios: las proyecciones y el perfil posicional se ajustan sobre el propio corpus, creando dependencia adicional que este contraste no modela.",
        "",
