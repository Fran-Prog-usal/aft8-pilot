...
from .alignment import map_tokens_to_verses, check_alignment, reconstruye
from .metrics.delta_p import delta_p_series
from .metrics.delta_e import entropy_series, entropy_norm, delta_e_series
from .metrics.surprisal import surprisal_series
from .metrics.topk import topk_logits
from .metrics.predictive_diagnostics import capa_diagnostica
from .metrics.gxa import salience_matrix, redistribution_series, mass_by_verse, raw_attention_received
from .model import MistralExtractor, MODEL_ID, REVISION
...
def read_inputs(path):
    ...
    for record in records:
        if set(record) != INPUT_KEYS:
            raise ValueError("La entrada debe contener solo identificador, texto y hash; no admite hipótesis.")
        ...
        if hashlib.sha256(record["text_content"].encode("utf-8")).hexdigest() != record["text_sha256"]:
...
def extract_record(adapter, record, run_id):
    ...
    alignment = map_tokens_to_verses(text, encoding["offsets"], encoding["tokens"], encoding["special_mask"])
    report = check_alignment(text, text.split("\n"), alignment)
    report.update(decode_exact=encoding["decode_exact"], offset_coverage=reconstruye(text, alignment))
    ...
    diagnostic = capa_diagnostica(cpu_logits)
    np.testing.assert_allclose(diagnostic["deltaP_full_T10"], delta_p, rtol=1e-6, atol=1e-8, equal_nan=True)
    salience, unnormalized = salience_matrix(attentions, None, ids[1:], logits, devolver_cruda=True)
    common, padded = redistribution_series(salience)
    mass = mass_by_verse(salience, np.array(alignment["line_id"]), 6)
    n = len(cpu_ids)
    rows = pd.DataFrame({"run_id": run_id, "model_key": "mistral", "record_id": record["record_id"],
                         "token_idx": np.arange(n), "token_id": cpu_ids.numpy(), "token_text": encoding["tokens"],
                         **alignment, "entropy_H": entropy, "entropy_H_norm": entropy_norm(entropy, logits.shape[-1]),
                         "delta_E": delta_e, "delta_P": delta_p, "delta_P_norm": delta_p / np.log(2),
                         "surprisal_observed": surprise, "gxa_redistribution": common, "gxa_redist_pad": padded,
                         "status": "OK", "notes": "", **diagnostic})
    ...
    arrays = {"tokens": np.asarray(encoding["tokens"], dtype=str),
              "line_id": np.asarray(alignment["line_id"]), "is_special": np.asarray(alignment["is_special"]),
              "is_newline": np.asarray(alignment["is_newline"]), "input_ids": cpu_ids.numpy(),
              "salience": salience.astype(np.float32), "salience_cruda": unnormalized.astype(np.float32),
              "gxa_redistribution": common, "gxa_redist_pad": padded, "gxa_mass_por_verso": mass,
              "attn_cruda": raw_attention_received(attentions), "delta_P": delta_p,
              "delta_P_norm": delta_p / np.log(2), "entropy_H": entropy, "delta_E": delta_e,
              "surprisal": surprise, **diagnostic, **topk_logits(cpu_logits, cpu_ids, k=50)}
    for index in (0, 8, 16, 24, len(hidden) - 1):
        if index < len(hidden):
            arrays[f"hidden_{index}"] = hidden[index][0].detach().float().cpu().numpy()
    audit = np.stack([attention[0].detach().float().cpu().numpy() for attention in attentions]).astype(np.float16)
    return rows, lines, arrays, report, audit


def assert_smoke(path, cohort, inputs_hash, code_hashes):
    manifest = json.loads(Path(path).read_text(encoding="utf-8"))
    if (manifest.get("status") != "complete" or manifest.get("cohort") != "smoke"
            or manifest.get("record_ids") != list(SMOKE_IDS)
            or manifest.get("model_revision") != REVISION or manifest.get("code_sha256") != code_hashes):
...
def run_extraction(inputs, output, cohort, smoke_manifest=None):
    ...
    for package, expected_version in {"torch": "2.8.0", "transformers": "5.17.0", "numpy": "2.1.2"}.items():
    ...
    expected = {f"A{i:03d}" for i in range(1, 31)} if cohort != "control" else {f"A{i:03d}D{j}" for i in range(1, 31) for j in (1, 2)}
    ...
    if cohort == "smoke":
        records = sorted([record for record in records if record["record_id"] in SMOKE_IDS], key=lambda r: r["record_id"])
    else:
        random.Random(20260902).shuffle(records)
    output.mkdir(parents=True, exist_ok=True)
    (output / "gxa_vectors").mkdir()
    (output / "attention_audit").mkdir()
    manifest = {"status": "running", "cohort": cohort, "model_id": MODEL_ID, "model_revision": REVISION,
                "tokenizer_revision": REVISION, "dtype": "bfloat16", "attention_backend": "eager",
                "input_sha256": input_hash, "code_sha256": hashes, "seed": 42, "order_seed": 20260902,
                "python": platform.python_version(), "open_issues": ["GxA support alignment remains open", "Historical smoke mismatch remains documented"],
                "versions": {name: importlib.metadata.version(name) for name in ("torch", "transformers", "tokenizers", "numpy", "pandas", "pyarrow")}}
    ...
    try:
        ...
        manifest.update(gpu=torch.cuda.get_device_name(), cuda=torch.version.cuda,
                        model_config={"hf_id": MODEL_ID, "revision": REVISION})
        (output / "environment.txt").write_text(subprocess.check_output([os.sys.executable, "-m", "pip", "freeze"], text=True), encoding="utf-8")
        audit_ids = set(sorted([r["record_id"] for r in records], key=lambda rid: hashlib.sha256(f"42:{rid}".encode()).hexdigest())[:int(np.ceil(len(records) * .05))])
        ...
        for record in records:
            ...
            if identifier in audit_ids:
                np.savez_compressed(output / f"attention_audit/{identifier}.npz", attention=attention, tokens=arrays["tokens"], line_id=arrays["line_id"])
            ...
            (output / "alignment_report.json").write_text(json.dumps(alignment, indent=2), encoding="utf-8")
            manifest["completed_record_ids"] = list(alignment)
            save()
            print(f"{len(alignment)}/{len(records)}: {identifier}", flush=True)
        manifest.update(status="complete", record_ids=sorted(alignment), n_texts=len(records),
                        n_tokens=sum(len(frame) for frame in token_frames), elapsed_seconds=time.perf_counter() - start,
                        artifact_sha256={p.relative_to(output).as_posix(): digest(p) for p in sorted(output.rglob("*")) if p.is_file() and p != manifest_path})
        save()
