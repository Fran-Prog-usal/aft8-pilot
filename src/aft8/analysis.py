...
MAPPING = {
    "delta_p": "deltaP_max",
    "delta_e": "deltaE_sum",
    "gxa": "gxa_redistribution_max",
    "gxa_pad": "gxa_redist_pad_max",
}
...
def hidden_baselines(archive, verses):
    ...
    arrays = {
        identifier: archive.vectors("original", identifier)
        for identifier in sorted(verses.record_id.unique())
    }
    ...
    projections = {
        ...
        "ica_hidden": FastICA(n_components=1, random_state=0, max_iter=1000).fit_transform(hidden)[
            :, 0
        ],
        "latent_compressed": np.linalg.norm(
            PCA(n_components=8, random_state=0).fit_transform(hidden), axis=1
        ),
    }
    rows, offset = [], 0
    for identifier, array in arrays.items():
        n = len(array["line_id"])
        differences = {
            name: np.r_[np.nan, np.abs(np.diff(value[offset : offset + n]))]
...
def hypothesis_scores(verses):
    ...
    for metric in METRICS:
        ...
        scores = scores[
            scores.design_family.map(lambda family: metric in FAMILY_TARGETS[family])
        ].copy()
        ...
        [
            {
                ...
                "null_p95": float(np.quantile(hits[:, mask].mean(axis=1), 0.95)),
                "seed": seed,
...
def gxa_description(verses, tokens, corpus):
    ...
    for identifier, frame in verses.groupby("record_id", sort=True):
        ...
        rows.append(
            {
                "record_id": identifier,
                "explicit_gxa_hypothesis": identifier in explicit,
                "expected_verse": expected,
                "peak_verse_all_six": int(frame.line_id.iloc[np.argmax(values)]),
                "target_percentile_all_six": float(scores[location]),
                "uniform_target_rank_reference": (len(values) + 1) / (2 * len(values)),
                "target_minus_mean_other_redistribution": float(
                    values[location] - np.delete(values, location).mean()
                ),
            }
        )
...
def factorial_diagnostics(tokens):
        ...
        [
            {
                ...
                "median_token_weighted": float(frame[column].median()),
            }
    ...
    summary = pd.DataFrame(
        [
            {
                "effect": name,
                **document_bootstrap(documents[name].to_numpy()),
                "mean_token_weighted": float(frame[name].mean()),
            }
            for name in effects
        ]
    )
    ...
    for variant in ("D1", "D2"):
        ...
        for first in (1, 2):
            for metric in METRICS:
                ...
                b = (
                    selected[selected.line_id >= first]
                    .groupby("line_id")[metric]
                    .mean()
                    .reindex(a.index)
                )
                rows.append(
                    {
                        ...
                        "metric": metric,
                        "first_verse": first,
                        "n_profile_positions": len(a),
                        "correlation_of_means": float(a.corr(b)),
                    }
                )
    ...
    with ResultArchive(archive_path, expected_sha256) as archive:
        ...
        for mode, data in (("raw", verses), ("residual", residual)):
            ...
            global_rows.append(
                {
                    "mode": mode,
                    "n_texts": table.record_id.nunique(),
                    "n_score_rows": len(table),
                    "hits": int(table.hit.sum()),
                    "hit_rate": float(table.hit.mean()),
        ...
        support = score_peaks(verses, "gxa").merge(
            score_peaks(verses, "gxa_pad"), on="record_id", suffixes=("_common", "_padded")
        )
        ...
        control_profiles(verses, control_verses).to_csv(
            output / "control_profile_sensitivity.csv", index=False
        )
        hypothesis_scores(control_verses[control_verses.variant == "D1"]).to_csv(
            output / "control_D1_raw_scores.csv", index=False
        )
        provenance = {
            ...
            "packages": {
                ...
                for name in ("numpy", "pandas", "scipy", "scikit-learn", "pyarrow", "matplotlib")
            },
            "code_sha256": {
            ...
            "open_issues": [
                "GxA support selection",
                "smoke A001/A002 versus A001/A013",
                "span annotations A004/A016",
            ],
            "limitations": [
                "Conditional permutation exchangeability is not established by this analysis.",
