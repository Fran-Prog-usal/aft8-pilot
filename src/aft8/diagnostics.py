...
from .spans import evaluate_span, matching_spans, normalized_word
...
def instrument_tables(verses, tokens):
            ...
            distribution.append(
                {
                    ...
                    "n_tokens": len(values),
                    "median_nats": float(np.median(values)),
                    "fraction_within_one_percent_of_ceiling": float(
                        np.mean(values >= 0.99 * np.log(2))
                    ),
                    "fraction_within_one_percent_of_floor": float(
                        np.mean(values <= 0.01 * np.log(2))
                    ),
    ...
    residual[list(METRICS)] = verses[list(METRICS)] - verses.groupby("line_id")[
        list(METRICS)
    ].transform("mean")
    ...
    for mode, frame in (("raw", verses), ("residual", residual)):
        for metric, column in (
            ("delta_p", "delta_P"),
            ("delta_e", "delta_E"),
            ("gxa", "gxa_redistribution"),
        ):
            lo, hi = np.quantile(valid[column].dropna(), [0.01, 0.99])
            ...
            resolution.append(
                {
                    "mode": mode,
                    "metric": metric,
                    "verse_statistic": {"delta_p": "max", "delta_e": "sum_signed", "gxa": "max"}[
                        metric
                    ],
                    "verse_range": float(width),
                    "token_p99_minus_p01": float(hi - lo),
                    "verse_range_over_token_percentile_range": float(width / (hi - lo))
                    if hi > lo
                    else np.nan,
            ...
            for name, center, scale in (
                ("zscore", np.mean(token_values), np.std(token_values, ddof=1)),
                (
                    "median_mad",
                    np.median(token_values),
                    1.4826 * np.median(np.abs(token_values - np.median(token_values))),
                ),
            ):
                if scale <= 0:
                    normalization.append(
                        {"mode": mode, "metric": metric, "transform": name, "status": "zero_scale"}
                    )
                    continue
...
def state_description(tokens, output, archive):
    ...
    frame = (
        tokens[~tokens.is_special]
        .dropna(subset=["delta_P", "delta_E", "gxa_redistribution"])
        .copy()
    )
    ...
    frame[["record_id", "token_idx", "line_id", "cluster", "pca_1", "pca_2"]].to_parquet(
        output / "states.parquet", index=False
    )
    selected = labels >= 0
    n_clusters = len(set(labels[selected]))
    silhouette = (
        float(silhouette_score(scaled[selected], labels[selected]))
        if 1 < n_clusters < selected.sum()
        else None
    )
    summary = {
        "n_states": len(frame),
        "n_clusters": n_clusters,
        "noise_fraction": float((~selected).mean()),
        "silhouette_excluding_noise": silhouette,
        "implementation": "sklearn.cluster.HDBSCAN",
        "min_cluster_size": 10,
        "min_samples": 10,
        "projection": "PCA_full_SVD",
        "inferential_p": None,
    ...
    try:
        ...
        for key in (
            "adjusted_rand_against_archived",
            "archived_noise_fraction",
            "archived_silhouette_on_current_standardization",
            "archived_labels_reproduced",
        ):
            summary[key] = None
    else:
        summary["archived_reference_available"] = True
        pairs = frame[["record_id", "token_idx", "cluster"]].merge(
            previous[["record_id", "token_idx", "cluster"]],
            on=["record_id", "token_idx"],
        ...
        summary["adjusted_rand_against_archived"] = float(
            adjusted_rand_score(pairs.cluster_current, pairs.cluster_archived)
        )
        ...
        summary["archived_silhouette_on_current_standardization"] = (
            float(silhouette_score(scaled[old_mask], old_labels[old_mask]))
            if 1 < old_clusters < old_mask.sum()
            else None
        )
        summary["archived_labels_reproduced"] = bool(
            summary["adjusted_rand_against_archived"] == 1.0
        )
        pairs.to_parquet(output / "cluster_label_comparison.parquet", index=False)
    (output / "clustering_descriptive.json").write_text(
        json.dumps(summary, indent=2) + "\n", encoding="utf-8"
    )
    fig, ax = plt.subplots(figsize=(7, 4.5), layout="constrained")
    ax.scatter(
        coordinates[~selected, 0], coordinates[~selected, 1], s=5, color="lightgray", label="Ruido"
    )
    ax.scatter(
        coordinates[selected, 0], coordinates[selected, 1], s=7, c=labels[selected], cmap="tab10"
    )
    ax.set(
        xlabel="Componente principal 1",
        ylabel="Componente principal 2",
        title="Estados de tokens: proyección PCA descriptiva",
    )
...
def plot_maps(identifier, tokens, matrix, destination):
    ...
    for verse, frame in tokens[~tokens.is_special].groupby("line_id", sort=True):
        ...
        ax.axvline(start - 0.5, color="white", linewidth=0.35)
        ax.axhline(start - 0.5, color="white", linewidth=0.35)
    ax.set_xticks(centers, labels)
    ax.set_yticks(centers, labels)
    ax.set(
        xlabel="Token de contexto que recibe saliencia",
        ylabel="Token de consulta",
        title=f"{identifier} · saliencia normalizada",
    )
...
def run_diagnostics(archive_path, corpus_path, config_dir, output, expected_sha256=ARCHIVE_SHA256):
    ...
    candidate = json.loads(
        (config_dir / "gxa_annotations_candidate.json").read_text(encoding="utf-8")
    )
    ...
    annotations = [
        {**record, "annotation_version": candidate["version"]} for record in candidate["records"]
    ]
    annotations += [
        {
            **record,
            "annotation_version": "archived-pairs-current-controls",
            "review_status": "archived_pair",
        }
    ...
    annotations += [
        {
            ...
            "source": source,
            "target": "pared",
            "annotation_version": "source-sensitivity",
            "review_status": "pending",
        }
        for source in ("pecho", "mano")
    ]
    annotations += [
        {
            "record_id": "A020",
            "source": None,
            "target": "devolverlo",
            "annotation_version": "surface-sensitivity",
            "review_status": "pending",
        }
    ]
                    ...
                    matching_audit.append(
                        {
                            **metadata,
                            ...
                            "legacy_substring_start": legacy_index,
    ...
    for identifier, data in curves[
        (curves.annotation_version == candidate["version"]) & curves.source.notna()
    ].groupby("record_id"):
        fig, ax = plt.subplots(figsize=(7, 4), layout="constrained")
        ax.plot(
            data.query_token_idx,
            data.source_mass,
            label=f"Origen: {data.source.iloc[0]}",
            color="#286682",
        )
        ax.plot(
            data.query_token_idx,
            data.target_mass,
            label=f"Destino: {data.target.iloc[0]}",
            color="#cf8c39",
        )
        boundary = spans[
            (spans.record_id == identifier) & (spans.annotation_version == candidate["version"])
        ].target_first_complete_query.iloc[0]
        ax.axvline(boundary, color="gray", linestyle="--", label="Destino completo")
        ax.set(
            xlabel="Índice del token de consulta",
            ylabel="Masa de saliencia normalizada",
            title=f"{identifier} · trayectoria exploratoria origen–destino",
        )
        ax.legend(frameon=False)
            ...
            summaries.append(
                {
                    ...
                    "scope": scope,
                    "n_annotation_rows": len(subset),
                    "n_transfers_with_target_controls": len(transfers),
                    "target_exceeds_controls": int(
                        transfers.target_exceeds_controls.fillna(False).astype(bool).sum()
                    ),
                    "source_decreases_more_than_controls": int(
                        transfers.source_decreases_more_than_controls.fillna(False)
                        .astype(bool)
                        .sum()
                    ),
    ...
    manifest = {
        "analysis_version": "diagnostics-1",
        "status": "exploratory",
        ...
        "numerical_stability_gpu": "not_tested",
        "semantic_specificity_control": "not_available",
    ...
    report = [
        "# Diagnósticos de saliencia e instrumentación",
        "",
    ...
    for item in summaries:
        report.append(
            f"| {item['annotation_version']} | {item['scope']} | {item['n_transfers_with_target_controls']} | {item['target_exceeds_controls']} | {item['source_decreases_more_than_controls']} |"
        )
    report += [
        "",
        "Las tablas detallan los casos no calculables y los denominadores. A011 se corrige: «sí» no debe localizarse dentro de «sin». En A020, «devolver» aparece como parte de «devolverlo»; la lectura literal queda sin correspondencia y la forma completa se conserva como alternativa exploratoria pendiente, sin sustituirla silenciosamente.",
        "",
        ...
        "",
        (
            f"Comparación con etiquetas archivadas: ARI={clustering['adjusted_rand_against_archived']:.6f}. Silhouette archivado sobre la estandarización actual: {clustering['archived_silhouette_on_current_standardization']:.6f}. Ambas versiones se conservan separadas; no se atribuye la diferencia a una biblioteca sin evidencia."
            if clustering["archived_reference_available"]
            else "El paquete no contiene etiquetas de clustering previas: no se realiza comparación archivada."
        ),
        "",
    ...
    report += [
        f"- [{record['record_id']}](figures/maps/{record['record_id']}.png)" for record in corpus
    ]
