...
class SpanTests(unittest.TestCase):
    ...
    def test_future_span_has_zero_mass_but_missing_query_is_nan(self):
        matrix = np.array([[1.0, np.nan, np.nan], [0.2, 0.8, np.nan], [np.nan] * 3])
        self.assertEqual(span_mass(matrix, 0, [1]), 0.0)
        self.assertEqual(span_mass(matrix, 1, [1]), 0.8)
    ...
    def test_transfer_windows_and_source_change(self):
        ...
        row, trajectory = evaluate_span(
            text, tokens, matrix, {"source": "madre", "target": "cuchillo"}, set()
        )
        ...
        self.assertAlmostEqual(row["source_mass_change"], -0.7)
    ...
    def test_missing_source_is_not_a_successful_transfer(self):
        ...
        row, _ = evaluate_span(
            "pan fin", tokens, matrix, {"source": "madre", "target": "pan"}, set()
        )
