...
from aft8.extraction import extract_record, read_inputs, assert_smoke
from aft8.model import MistralExtractor, validate_attention, tokenize_exact
...
class ExtractionGuards(unittest.TestCase):
    def test_rejects_hypotheses_in_input(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            text = "a\nb\nc\nd\ne\nf"
            record = {"record_id": "A001", "text_content": text,
                      "text_sha256": hashlib.sha256(text.encode()).hexdigest(), "expected_verse": 6}
    ...
    def test_old_smoke_cannot_unlock_full_run(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({"status": "complete", "cohort": "smoke", "record_ids": ["A001", "A002"]}))
...
class TinyMistralTest(unittest.TestCase):
    def test_teacher_forcing_attention_gradients_and_serialization(self):
        ...
        tokenizer.train_from_iterator([text], trainers.BpeTrainer(vocab_size=300,
            initial_alphabet=pre_tokenizers.ByteLevel.alphabet(), special_tokens=["<unk>", "<s>", "</s>"]))
        fast = PreTrainedTokenizerFast(tokenizer_object=tokenizer, unk_token="<unk>", bos_token="<s>", eos_token="</s>")
        previous_threads = torch.get_num_threads()
        torch.set_num_threads(1)
        try:
            torch.manual_seed(7)
            config = MistralConfig(vocab_size=len(fast), hidden_size=16, intermediate_size=32,
                                    num_hidden_layers=2, num_attention_heads=2, num_key_value_heads=1,
                                    max_position_embeddings=256, sliding_window=None)
            config._attn_implementation = "eager"
            model = MistralForCausalLM(config)
            adapter = MistralExtractor(model, fast, device="cpu")
            tokens, lines, arrays, report, attention = extract_record(adapter, {"record_id": "A001", "text_content": text}, "synthetic")
