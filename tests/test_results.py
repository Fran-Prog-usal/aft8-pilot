...
class ResultTests(unittest.TestCase):
    ...
    def test_vectors_do_not_require_object_deserialization(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            path, digest = self.archive(root, "results/pilot30_aft8_v1__mistral/gxa_vectors/A001.npz", blob.getvalue())
    ...
    def test_offsets_distinguish_missing_space_from_missing_content(self):
        tokens = pd.DataFrame({"token_idx": [0, 1], "char_start": [0, 2],
                               "char_end": [1, 3], "is_special": [False, False],
                               "line_id": [1, 1], "is_newline": [False, False]})
    ...
    def test_aggregation_excludes_specials_includes_newline_and_preserves_sign(self):
        tokens = pd.DataFrame({"token_idx": [0, 1, 2], "line_id": [0, 1, 1],
                               "is_special": [True, False, False], "is_newline": [False, False, True]})
        for name in ["delta_P", "delta_E", "gxa_redistribution", "gxa_redist_pad", "deltaP_top100_T10"]:
            tokens[name] = [100., 2., -3.]
        salience = np.array([[1., np.nan, np.nan], [.25, .75, np.nan], [np.nan] * 3])
        row = aggregate_lines(tokens, salience).loc[1]
        self.assertEqual(row.n_tokens, 2)
        self.assertEqual(row.deltaE_sum, -1.)
        self.assertEqual(row.deltaE_absmax, 3.)
        self.assertEqual(row.deltaP_max, 2.)
        self.assertEqual(row.gxa_mass_received, .375)
